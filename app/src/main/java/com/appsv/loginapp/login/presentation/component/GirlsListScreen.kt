...
@Composable
fun GirlsListScreen(navController: NavController,pgScreenState: AdminScreenState) {
    var currentUser by remember { mutableStateOf<Users?>(null) }
    var showUserProfileDialog by remember { mutableStateOf(false) }
    var searchQuery by remember { mutableStateOf("") }
    var showFilterMenu by remember { mutableStateOf(false) }
    var selectedPaymentStatusStatus by remember { mutableStateOf<PaymentStatus?>(null) }
    ...
    val filteredUsers = pgScreenState.usersList.filter { user ->
        ...
        val matchesRole = selectedPaymentStatusStatus?.let { user.isPaid == it.name } ?: true

        matchesSearch && matchesRole && user.role == Role.USER.name && user.gender==Gender.FEMALE.name && user.occupation==Occupation.STUDENT.name
    }
    ...
    Column {
            ...
            Box {
                IconButton(
                    ...
                    modifier = Modifier.size(55.dp)
                        .background(
                            color = if (selectedPaymentStatusStatus != null) colorResource(id=R.color.black)
                    ...
                    Icon(
                        ...
                        tint = if (selectedPaymentStatusStatus != null) colorResource(id=R.color.gray)
                ...
                DropdownMenu(
                    ...
                    DropdownMenuItem(
                        text = { Text("EVERYONE") },
                        onClick = {
                            selectedPaymentStatusStatus = null
                            showFilterMenu = false
                        }
                    )
                    PaymentStatus.entries.forEach { paymentStatus ->
                        DropdownMenuItem(
                            text = { Text(paymentStatus.name) },
                            onClick = {
                                selectedPaymentStatusStatus = paymentStatus
                                showFilterMenu = false
                            }
                        )
                    }
        ...
        if (selectedPaymentStatusStatus != null) {
            Row(
                modifier = Modifier
                    .fillMaxWidth()
                ...
                Text(
                    text = "Filter: ${selectedPaymentStatusStatus!!.name}",
        ...
        if (pgScreenState.isLoading) {
            ...
            EmptyState(
                message = when {
                    searchQuery.isNotEmpty() -> "No members found for '$searchQuery'"
                    selectedPaymentStatusStatus != null -> "No ${selectedPaymentStatusStatus!!.name}  members found"
